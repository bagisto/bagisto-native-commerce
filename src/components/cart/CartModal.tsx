...
import TurboCartModalBridge from "@components/hotwire/TurboCartModalBridge";
...
export default function CartModal({
  ...
  useEffect(() => {
    ...
    sendCartCountToNativeApp(Number(cartDetail?.cart?.itemsQty));
  }, [cartDetail?.cart?.itemsQty]);
  ...
  const shouldRespondToBridge = isDesktop ? !isBottomNavbar : isBottomNavbar;

  return (
    <>
      {shouldRespondToBridge && (
        <TurboCartModalBridge
          isOpen={finalIsOpen}
          ...
          {finalIsOpen && (
            <>
              <motion.div
                ...
                exit={{ opacity: 0 }}
                onClick={finalOnClose}
                className={`fixed inset-0 z-40 bg-transparent transition-opacity ${!isTurboNativeUserAgentState ? " lg:hidden" : ""}`}
              ...
              <motion.div
                ...
                className={`fixed right-0 z-50 flex flex-col bg-white dark:bg-black w-full max-w-[448px] border-l border-neutral-200 dark:border-neutral-800 ${!isTurboNativeUserAgentState ? " lg:hidden" : ""}`}
                style={{
